# Flow Notes — Local Helper (v0.6.0)
...
`note.open` hands the file to the operating system — `open` on macOS,
`xdg-open` on Linux, `start` on Windows — so clicking a note in the
extension opens it in whatever handles Markdown. Pass `reveal: true` to
show it in Finder instead.

`-editor` (or `FLOW_NOTES_EDITOR`) names a specific application:

```
go run . -serve -editor "Visual Studio Code"
go run . -serve -editor "Obsidian"
```

Empty means the system default for `.md`. The value is an application
name, as `open -a` expects.

Like every other path from the browser, it goes through `resolveExisting`
first. Handing an unvalidated path to the OS open command would let a
crafted request launch anything on the machine, so that check is what
separates opening a note from arbitrary execution. Arguments are passed as
a list and never through a shell.
