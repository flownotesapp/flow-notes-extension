...
// OpenNote hands a path to the operating system's "open" command, so the
// containment check is the difference between opening a note and letting a
// crafted request launch anything on the machine.
func TestOpenNoteRejectsPathsOutsideNotesDir(t *testing.T) {
	s := newTestStore(t)
	outside := filepath.Join(t.TempDir(), "victim.md")
	...
	for name, path := range map[string]string{
		...
		"traversal":          filepath.Join(s.Dir(), "..", "victim.md"),
		"non-markdown":       filepath.Join(s.Dir(), "notes.txt"),
		"missing":            filepath.Join(s.Dir(), "nope.md"),
		"empty":              "",
	} {
		t.Run(name, func(t *testing.T) {
			if err := s.OpenNote(path, "", false); err == nil {
				t.Errorf("expected %q to be rejected before reaching the OS", path)
			}
		})
	}
}
