...
type API struct {
	...
	// editorApp names the application used to open Markdown notes. Empty
	// means whatever the OS considers the default for the file type.
	editorApp string
}

func NewAPI(store *Store, apple AppleNotesStore, editorApp string) *API {
	return &API{store: store, apple: apple, editorApp: editorApp}
}
...
func (a *API) Dispatch(method string, raw json.RawMessage) (map[string]any, error) {
	...
	case MethodNoteOpen:
		if err := a.store.OpenNote(p.Path, a.editorApp, p.Reveal); err != nil {
			return nil, fail(CodeInvalid, err)
		}
		return map[string]any{"ok": true}, nil
