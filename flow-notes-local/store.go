...
// OpenNote hands a note to the operating system to open — in whatever
// application handles Markdown, or in a named one.
//
// Routed through resolveExisting like every other path from the browser:
// handing an unvalidated path to the OS "open" command would let a crafted
// request launch anything on the machine.
func (s *Store) OpenNote(path, editorApp string, reveal bool) error {
	safe, err := s.resolveExisting(path)
	if err != nil {
		return err
	}
	...
	case "darwin":
		switch {
		case reveal:
			cmd = exec.Command("open", "-R", safe) // show it in Finder
		case editorApp != "":
			cmd = exec.Command("open", "-a", editorApp, safe)
		default:
	...
	// Arguments are passed as a list, never through a shell, so a filename
	// containing spaces or shell metacharacters is inert.
	if out, err := cmd.CombinedOutput(); err != nil {
		...
		if editorApp != "" {
			return fmt.Errorf("could not open the note in %q: %s", editorApp, message)
		}
		return fmt.Errorf("could not open the note: %s", message)
	}
	return nil
}
