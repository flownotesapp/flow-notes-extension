...
func main() {
	var (
		...
		editorFlag = flag.String("editor", envOr("FLOW_NOTES_EDITOR", ""), "application used to open Markdown notes (empty = the system default for .md)")
	...
	if *editorFlag != "" {
		log.Printf("Markdown notes open in %q", *editorFlag)
	}
	api := NewAPI(store, apple, *editorFlag)
